...
public class StudentController {
        ...
        model.addAttribute("student", new Student()); return "formStudents"; }
    ...
    public String editStudents(Model model, Long id, HttpSession session){
        ...
        Student student = studentRepository.findById(id).orElse(null); if(student==null) throw new RuntimeException("Patient does not exist"); model.addAttribute("student", student);
        return "editStudents";
    }
    @GetMapping(path = "/")
    public String students2(Model model, ModelMap mm, @RequestParam(name="keyword",defaultValue = "") String keyword,HttpSession session){
        List<Student> students; if (keyword.isEmpty()) {
            students = studentRepository.findAll();
        } else {
            mm.put("e", 0);
            mm.put("a", 0);
